...
Usage:
    python scripts/cost_optimizer.py --region us-east-1 --days 7 --output report.txt
    python scripts/cost_optimizer.py --mock  # Run with mock data for demo
"""
...
Examples:
  ...
  python scripts/cost_optimizer.py --region ap-south-1 --cpu-threshold 10 --format json
        """
    )

    parser.add_argument('--region', '-r', default='us-east-1', help='AWS region to scan (default: us-east-1)')
    parser.add_argument('--profile', help='AWS credentials profile name')
    parser.add_argument('--aws-access-key-id', help='AWS access key ID for direct credentials')
    parser.add_argument('--aws-secret-access-key', help='AWS secret access key for direct credentials')
    parser.add_argument('--aws-session-token', help='AWS session token for temporary credentials')
    parser.add_argument('--days', '-d', type=int, default=7, help='Number of days for metrics analysis (default: 7)')
    parser.add_argument('--cpu-threshold', type=float, default=5.0, help='CPU threshold for idle detection in %% (default: 5%%)')
    ...
    parser.add_argument('--format', '-f', choices=['text', 'json', 'csv', 'summary'], default='text', help='Output format (default: text)')
